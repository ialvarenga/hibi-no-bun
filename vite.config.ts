    ...
    configureServer(server) {
      server.middlewares.use("/api/generate", (req, res) => {
        ...
        req.on("end", async () => {
          ...
          try {
            const { topics, theme, recentTopics, jlptLevel, share } =
              JSON.parse(body || "{}");
            if (!Array.isArray(topics) || topics.length === 0 || !theme) {
              res.statusCode = 400;
              res.end(
                JSON.stringify({
                  error: "Corpo inválido: esperado { topics, theme }",
                }),
              );
              return;
            }
            ...
            const { generateReading } =
              await import("./api/_lib/generateReading.js");
            const reading = await generateReading(
              topics,
              theme,
              apiKey,
              Array.isArray(recentTopics)
                ? recentTopics.filter((t) => typeof t === "string")
                : [],
              typeof jlptLevel === "string" && jlptLevel.trim()
                ? jlptLevel.trim()
                : undefined,
            );
            ...
            if (share === true) {
              const { saveSharedEntry } =
                await import("./api/_lib/sharedDb.js");
              saveSharedEntry({
                ...reading,
                ...
                console.error("dev saveSharedEntry error:", err),
              );
            }
          } catch (err) {
      ...
      server.middlewares.use("/api/shared", async (req, res) => {
        ...
        if (req.method === "DELETE") {
          try {
            ...
            if (!ok) {
              ...
              res.end(
                JSON.stringify({
                  error: "Pergunta compartilhada não encontrada.",
                }),
              );
              return;
            }
        ...
        try {
          const { getRandomShared, getSharedById, listSharedEntries } =
            await import("./api/_lib/sharedDb.js");
          if (!id && url.searchParams.has("list")) {
            ...
            const jlptLevels = (url.searchParams.get("jlptLevel") ?? "")
              .split(",")
              .map((l) => l.trim())
              .filter(Boolean);
          ...
          if (!entry) {
            ...
            res.end(
              JSON.stringify({
                error: "Nenhuma pergunta compartilhada disponível ainda.",
              }),
            );
            return;
          }
      ...
      server.middlewares.use("/api/admin-session", async (req, res) => {
        ...
        try {
          const { verifyPassword, createSessionCookie, clearSessionCookie } =
            await import("./api/_lib/adminAuth.js");
          if (req.method === "DELETE") {
      ...
      server.middlewares.use("/api/feedback", async (req, res) => {
        ...
        try {
          const {
            ...
            setFeedbackResolved,
            deleteFeedback,
          } = await import("./api/_lib/feedbackDb.js");
          const { isAuthed } = await import("./api/_lib/adminAuth.js");
          const { sendFeedbackEmail } = await import("./api/_lib/notify.js");
          const { FEEDBACK_CATEGORIES, FEEDBACK_SOURCES } =
            await import("./api/_lib/constants.js");
          ...
          if (req.method === "POST") {
            ...
            if (
              ...
              !FEEDBACK_CATEGORIES.includes(body.category)
            ) {
              res.statusCode = 400;
            ...
            if (
              ...
              !FEEDBACK_SOURCES.includes(body.source)
            ) {
              res.statusCode = 400;
            ...
            const row = await saveFeedback({
              ...
              source: body.source,
              sharedEntryId:
                typeof body.sharedEntryId === "string"
                  ? body.sharedEntryId
                  : null,
          ...
          if (req.method === "PATCH") {
            ...
            if (typeof id !== "string" || typeof resolved !== "boolean") {
              ...
              res.end(
                JSON.stringify({
                  error: "Corpo inválido: esperado { id, resolved }",
                }),
              );
              return;
            }
          ...
          if (req.method === "DELETE") {
            ...
            if (typeof id !== "string") {
              res.statusCode = 400;
              res.end(
                JSON.stringify({ error: "Corpo inválido: esperado { id }" }),
              );
              return;
            }
